 ...
 **               (std::numeric_limits<int>::max() by default).
...
/*********************************************************************
 ** Function: get_pos_double
 ** Description: Ouputs a prompt and gets a double from the user until
 **   the entered value is a positive number less than or equal to the
 **   specified maximum input value.
 ** Parameters: const string &prompt - the prompt to be repeatedly output.
 **             double *val - a pointer to a double in the caller where
 **               the entered value is stored.
 **             double max_input - the maximum acceptable input value
 **               (std::numeric_limits<double>::max() by default).
 ** Pre-Conditions: max_input is a positive number. val is not a null
 **   pointer.
 ** Post-Conditions: val points to a double in the range (0, max_input].
 ** Return: N/A
 *********************************************************************/
void get_pos_double(const string &prompt, double *val, double max_input) {
    *val = -1;
    while (*val <= 0 || *val > max_input) {
        cout << prompt;
        cin >> *val;
...
/*********************************************************************
 ** Function: get_nonneg_double
 ** Description: Ouputs a prompt and gets a double from the user until
 **   the entered value is a non-negative number between 0 and the specified
 **   maximum input value.
 ** Parameters: const string &prompt - the prompt to be repeatedly output.
 **             double *val - a pointer to a double in the caller where
 **               the entered value is stored.
 **             double max_input - the maximum acceptable input value
 **               (std::numeric_limits<double>::max() by default).
 ** Pre-Conditions: max_input is a non-negative number. val is not a null
 **   pointer.
 ** Post-Conditions: val points to a double in the range [0, max_input].
 ** Return: N/A
 *********************************************************************/
void get_nonneg_double(const string &prompt, double *val, double max_input) {
    *val = -1;
    while (*val < 0 || *val > max_input) {
        cout << prompt;
        cin >> *val;
